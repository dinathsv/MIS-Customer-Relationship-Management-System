...
const saveUser = async () => {
  try {
    ...
    await axios.put(`/api/v1/users/${editData.value.user_id}`, editData.value, {
      headers: { Authorization: `Bearer ${token}` }
    })
...
const saveProfile = async () => {
  try {
    const token = localStorage.getItem('crm_token')
    await axios.put('/api/v1/users/me/profile', profileData.value, {
      headers: { Authorization: `Bearer ${token}` }
    })
...
const savePassword = async () => {
  ...
  try {
    const token = localStorage.getItem('crm_token')
    await axios.put('/api/v1/users/me/password', passwordData.value, {
      headers: { Authorization: `Bearer ${token}` }
    })
